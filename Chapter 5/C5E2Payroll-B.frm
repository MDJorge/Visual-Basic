...
Begin VB.Form frmC5E2Payroll 
   ...
   Begin VB.Frame fraExempt 
      ...
      Begin VB.OptionButton optNotTaxExempt 
         ...
         Width           =   975
      End
      Begin VB.OptionButton optTaxExempt 
         BackColor       =   &H0080C0FF&
         Caption         =   "Yes"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
...
Dim intHours, intRate, intOvertime As Integer
...
Private Sub btnCalc_Click()
    If (IsNumeric(txtHours) And IsNumeric(txtRate)) Then
        intHours = txtHours
        intRate = txtRate
        If optTaxExempt Then
            If txtHours <= 40 Then
                lblPay = intHours * intRate
            Else
                intOvertime = (intHours - 40) * (intRate / 2)
                lblPay = intHours * intRate + intOvertime
            End If
        Else
            If txtHours <= 40 Then
                dblTaxes = lblPay * 0.18
                lblPay = (intHours * intRate) - dblTaxes
            Else
                dblTaxes = (intHours - 40) * (intRate / 2) * 0.18
                intOvertime = (intHours - 40) * (intRate / 2)
                lblPay = (intHours * intRate + intOvertime) - lblPay
            End If
        End If
    Else
...
Private Sub optNotTaxExempt_Click()
    dblTaxes = lblPay * 0.18
    lblPay = (intHours * intRate) - dblTaxes
End Sub
